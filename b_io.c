...
int b_write(b_io_fd fd, char *buffer, int count)
{
    ...
    if ((count + amountAlreadyDelivered) > fcbArray[fd].fi->size)
    {
        ...
        int blocksNeeded =
            ((count + amountAlreadyDelivered) - fcbArray[fd].fi->size + (B_CHUNK_SIZE - 1) * 2) /
            B_CHUNK_SIZE;
    ...
    if (part2 > 0)
    {
        bytesRead = LBAwrite(buffer + part1, numberOfBlocksToCopy,
                             fcbArray[fd].currentBlk + fcbArray[fd].fi->loc);
